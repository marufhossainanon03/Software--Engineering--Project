...
?>
<h5 align="center" style="color:blue">Home Work Report from <?php echo $fdate?> to <?php echo $tdate?></h5>
                    </div>
...
$sql="SELECT tblclass.ID,tblclass.ClassName,tblclass.Section,tblhomework.homeworkTitle,tblhomework.postingDate,tblhomework.lastDateofSubmission,tblhomework.id as hwid from tblhomework join tblclass on tblclass.ID=tblhomework.classId  where date(postingDate) between '$fdate' and '$tdate' LIMIT $offset, $no_of_records_per_page";
$query = $dbh -> prepare($sql);
$query->execute();
                          ...
                          <tr>
                            ...
                            <td>
                              <div><a href="edit-homework.php?hwid=<?php echo htmlentities ($row->hwid);?>" class="btn btn-info btn-xs"  target="blank">Edit</a>
                                               <a href="manage-homeworks.php?delid=<?php echo ($row->hwid);?>" onclick="return confirm('Do you really want to Delete ?');" class="btn btn-danger btn-xs"> Delete</a>
